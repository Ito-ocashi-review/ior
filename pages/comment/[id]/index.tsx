...
const Comment: React.FC<Props> = ({ reviews }) => {
      ...
      >
        <Grid container item xs={6} spacing={3}>
          <Card key={review._id} className={classes.card}>
            ...
            <CardContent>
              <Rating precision={0.1} readOnly value={review.evaluation} />
            </CardContent>
            <CardContent>
  ...
  return <div className={classes.root}>{reviewsList}</div>;
};

export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  const res = await Axios.get(
    urljoin(process.env.API_ROUTES_BASE_PATH, '/api/reviews/', params.id)
  );
  const reviews = res.data.reviews;
  const reviewsWithUserName = await Promise.all(
    reviews.map(async (review) => {
      const user = await Axios.get(
        urljoin(process.env.API_ROUTES_BASE_PATH, '/api/users', review.userId)
      );
